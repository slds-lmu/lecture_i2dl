...
\begin{alltt}
 ...
 \hlstd{x} \hlkwb{=} \hlkwd{unlist}\hlstd{(train[}\hlnum{1}\hlstd{,} \hlopt{-}\hlkwd{ncol}\hlstd{(train)])} \hlopt{/} \hlnum{255}
 \hlstd{i} \hlkwb{=} \hlkwd{as.cimg}\hlstd{(}\hlkwd{array}\hlstd{(x,} \hlkwd{c}\hlstd{(}\hlnum{32}\hlstd{,} \hlnum{32}\hlstd{,} \hlnum{1}\hlstd{,} \hlnum{3}\hlstd{)))}
 \hlkwd{plot}\hlstd{(i)}
\end{alltt}
\end{kframe}
\end{knitrout}
