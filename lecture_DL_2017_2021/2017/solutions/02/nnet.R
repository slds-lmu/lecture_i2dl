...
network = R6Class("network",
  public = list(
    ...
    p = NULL,
    ### CONSTRUCTOR ###
    initialize = function(data, target, neurons, activation, output.act, learning.rate, loss) {
    ...
    train = function(epochs, batch.size = 4) {
      for(e in seq_len(epochs)) { # Start Epoch 1 here
        ...
        for (batch in batches) { # Iterate over Batches
          ...
          self$output.weights = self$output.weights - self$learning.rate * (reduce(extractSubList(updates, "output.weight.update", simplify = FALSE), `+`) / batch.size)
          self$weights = self$weights - self$learning.rate * (reduce(extractSubList(updates, "weight.update", simplify = FALSE), `+`) / batch.size)
        }
...
net = network$new(data = data, target = target, neurons = 5, activation = relu, output.act = sigmoid, learning.rate = 0.01, loss = L2)
