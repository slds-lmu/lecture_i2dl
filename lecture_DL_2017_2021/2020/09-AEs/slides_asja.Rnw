...
\begin{vbframe}{Unsupervised Learning - Examples}
  ...
  \begin{figure}
    ...
    \caption{\footnotesize{Different cluster analysis results on a dataset. True labels (the colors in the original data) are shown here but the algorithms only operate on unlabeled data. (Source: Wikipedia)}}
  \end{figure}
\framebreak
  \begin{itemize}
    \item[2.] Dimensionality reduction/manifold learning
    \ \begin{itemize}
...
\begin{vbframe} {Unsupervised Learning - Examples}
  \begin{itemize}
    \item[3.] Feature extraction/representation learning
    \item[]
  \end{itemize}
   \begin{figure}
...
\framebreak
  \begin{itemize}
    \item[4.] Density fitting/learning a generative model
    \item[]
  \end{itemize}
   \begin{figure}
...
\begin{vbframe}{Autoencoder (AE)-task and structure}
  ...
  \item Task: Compression and reconstruction of the input.     
  \item They consist of two parts:
  \begin{itemize}
            \item \textbf{encoder} function $\mathbf{z} = enc(\xv)$.
            \item \textbf{decoder} that produces the reconstruction ${\mathbf{r}} = dec(\mathbf{z})$.
  \end{itemize}
    \item Loss function: 
    $$
...
\begin{frame}{Undercomplete Autoencoders}
  \begin{itemize}
    \only<1>{\item Learning simply the identity $dec(enc(\xv)) = \xv$ is not useful. }
    \only<1>{\item Therefore, we restrict the architecture, such that 
        $$
          \text{dim}(\mathbf{z}) < \text{dim}(\xv)
        $$
        }
    \only<1>{\item Such an AE is called \textbf{undercomplete}. }        
    \only<2>{\item In other words: In an undercomplete AE, the hidden layer has fewer neurons than the input layer. }
    \only<2>{$\Rightarrow$ That will force the AE to 
      \begin{itemize}
 ...
 %   \item Consequently, the undercomplete autoencoder tries to learn a \enquote{compressed} representation of the input.
  \end{itemize}
    ...
    \begin{figure}
    \centering
    \includegraphics[width=4cm]{plots/autoencoder_simple_example.png}
...
\begin{vbframe}{Undercomplete Autoencoders}
  ...
  % \end{itemize}    
    ...
    Training an AE is done by minimizing the risk, where the loss function penalizes the reconstruction  $dec(enc(\xv))$ for differing from $\xv$. The MSE 
    $$
    \|\xv-dec(enc(\xv))\|^2_2
    $$
    is a typical choice.
...
%    \item[]
 ...
 %   \item How could a potential architecture of an undercomplete autoencoder look like for our (very) simple example?
  ...
  \begin{itemize}
  %\item linear encoder $enc(\xv) = \mathbf{F}$
  \item  linear decoder $dec(\pmb{z})= \mathbf{H} \pmb{z}$
  \item  squared error loss $L=||\xv-dec(enc(\xv))||^2_2$
  \item  input normalized to have zero mean,
  \end{itemize}
  the optimal encoder corresponds to \textbf{Principal Component Analysis (PCA)}.
...
\framebreak  
 ...
 \begin{figure}
  ...
  \item Problem: If an AE is \textbf{overcomplete} %(code dimension $>$ input dimension)
  ($ \text{dim}(\mathbf{z}) > \text{dim}(\xv)$) or encoder and decoder are too powerful, the AE can learn to simply copy the input.
  \end{itemize}
...
\begin{frame}{Experiment: Learn to encode MNIST}
  \begin{itemize}
    ...
    \item[] $\to$ as few as possible dimensions in the internal representation $\mathbf{z}$.
  \end{itemize}
  \begin{figure}
    \centering
    \includegraphics[width=11cm]{plots/autoencoder_mnist_problem.png}
    \caption{Flow chart of our our autoencoder: reconstruct the input with fixed dimensions $dim(\mathbf{z}) << dim(\xv)$.}
  \end{figure}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{vbframe}{DAE-a probabilistic perspective}
  ...
  \begin{itemize}
   \item All we have to do to transform an AE into a DAE is to add a stochastic corruption process on the input.
    \item The DAE still tries to preserve the information about the input (encode it), but also to undo the effect of a corruption process!
     \item Training a DAE  by minimizing  $||dec(enc(\tilde{\xv})) - \xv||^2$ corresponds to minimizing $ \mathbb{E}_{\xv,\tilde{\xv}\sim p_{data}(\xv)C(\tilde{\xv}|\xv)}[\log p_{decoder}(\xv|enc(\tilde{\xv}))]$.
  \end{itemize} 
\framebreak
  \begin{figure}
    \centering
    % \includegraphics[width=4.5cm]{plots/denoising_autoencoder.png}
    \scalebox{0.5}{\includegraphics{plots/denoising_autoencoder.png}}
    \caption{Denoising autoencoders - \enquote{manifold perspective} (Ian Goodfellow et al. (2016))}
  \end{figure}
  \begin{itemize}
    \item A DAE is trained to map a corrupted data point $\tilde{\xv}$ back to
...
%  \begin{itemize}
%    \item The corruption process $C(\tilde{\xv}|\xv)$ is displayed by the gray circle of equiprobable corruptions
%  %  \item Training a DAE  by minimizing  $||dec(enc(\tilde{\xv})) - \xv||^2$ corresponds to minimizing $\mathbb{E}_{\xv,\tilde{\xv}\sim p_{data}(\xv)C(\tilde{\xv}|\xv)}[\log p_{decoder}(\xv|f(\tilde{\xv}))]$.
%  \end{itemize}
\framebreak
\begin{figure}
    \centering
    \includegraphics[width=5.cm]{plots/denoising_autoencoder.png}
    \caption{Denoising autoencoders - \enquote{manifold perspective} (Ian Goodfellow et al. (2016))}
  \end{figure}
  \begin{itemize}
    ...
    \item Thus, the DAE learns a vector field $dec(enc(\xv)) - \xv$ indicated by the green arrows.
  \end{itemize}
...
\frametitle{Experiment: Encode MNIST with a DAE}
  ...
  <<mxnet2, size = "footnotesize", cache = TRUE, eval = FALSE, echo = FALSE>>=
...
\begin{frame}[t]{Contractive Autoencoder (CAE}
   \begin{itemize}
