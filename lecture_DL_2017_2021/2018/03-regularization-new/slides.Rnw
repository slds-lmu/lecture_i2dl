...
\begin{frame} {Regularization}
  \begin{itemize}
    \item Recall, the goal of regularization is to penalize the complexity of the model in the risk cost function to minimize the chances of overfitting.
    \item Regularization is important in DL because NNs can have extremely high capacity (millions of parameters).
    \item In the very first lecture, we studied an important regularization technique : parameter norm penalties. However, regularization is much broader than this. 
...
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{vbframe}{Revision : Parameter Norm Penalties}
  \begin{itemize}
%    \item Norm penalization aims to limit the complexity of the model.
    \item By adding a parameter norm penalty term \(J(\theta)\) to the empirical risk $\risket$ we obtain a regularized cost function:
...
\begin{itemize}
    ...
    \item We now prove that this is equivalent to reducing weights in each gradient descent iteration.
\end{itemize}

\framebreak

Let's look at the GD formula for:
...
very well-known technique in deep learning - simply L2-regularization
in disguise.
...
% \end{enumerate}
  ...
  \begin{figure}
    \centering
      \scalebox{0.9}{\includegraphics{plots/weight_decay_3.png}}
  \end{figure}
\end{vbframe}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\framebreak
  ...
  \begin{enumerate}
    \item Split training data $\Dtrain$ into $\Dsubtrain$ and $\Dval)$ (e.g. with a ratio of 2:1).
    \item Use $\Dsubtrain$ and evaluate model using the validation set $\Dval$.
...
\framebreak
  \begin{table}
    ...
    \hline
    Combinable with other regularization methods & Less data for training $\rightarrow$ include $\Dval$ afterwards
    \end{tabular}
  \end{table}
  \begin{itemize}
    \item Relation between optimal early stopping iteration $T_{\text{stop}}$ and weight decay penalization parameter $\lambda$ for learning rate $\alpha$ (see Goodfellow et al. (2016) page 251-252 for proof):
  \end{itemize}
    \begin{equation*}
...
\begin{vbframe}{Dropout}
  \begin{itemize}
    \item Dropout is a regularization technique for reducing overfitting in neural networks by preventing complex co-adaptations amongst neurons.
...
%        \State Compute the gradient of the loss for $network_{\mu}$
    ...
    \end{algorithmic}
  \end{algorithm}
  \item \small{The gradients for each parameter are averaged over the training cases in each mini-batch. Any training case which does not use a parameter contributes a gradient of zero for that parameter.}
  \end{itemize}
\end{frame}
 ...
 \begin{frame} {Adversarial Examples}
   \begin{itemize}
     ...
     \item The performance of the same model can be drastically different on samples from a completely different distribution (on the same input space).
      \item It is possible to make changes to an image that makes a pretrained CNN (a type of neural network) output a completely different predicted class even though the change is imperceptible to the human eye.
  ...
  \begin{frame} {Adversarial Examples}
    \begin{itemize}
      ...
      \item Example : Fooling autonomous cars into thinking that a stop sign is a 45 kmph sign.
