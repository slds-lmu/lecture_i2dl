...
\begin{frame} {RNNs - Introduction}
  \begin{itemize}
    ...
    When the words in a sentence are fed to the network one at a time, the inputs are no longer independent.
...
%   \only<2-3>{\includegraphics[width=7cm]{plots/rnn_comp2.png}}%
  ...
  \vspace{-0.2cm}
  \begin{itemize}
    \only<1>{\item On the left is the computational graph for the dense net above. A loss function $L$ measures how far each output
...
\begin{frame}
\frametitle{Gated Recurrent Units (GRUs)}

Todo: Add image/summary of GRUs 
