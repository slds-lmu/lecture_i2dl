...
\begin{frame} {RNNs - Introduction}
  \begin{itemize}
    \item It's important to note that when the words in a sentence are fed to the network one at a time, the inputs are no longer independent.
    \item For example, it is much more likely that the word "good" is followed by "morning" rather than "plastic".
    \item Even though we've decided to feed a single word at a time, each word must still be encoded as a fixed-length vector because the size of the input layer will remain fixed.
  ...
  \end{figure}
    \item However, the standard approach is to use \textbf{word embeddings} (more on this later).
  \end{itemize}
\end{frame}
...
\begin{frame} {RNNs - Introduction}
  \begin{itemize}
    ...
    \item Therefore, by induction, it contains a "summary" of \textit{all} previous inputs. 
  \end{itemize}
\end{frame}

\begin{frame} {RNNs - Sentiment Analysis}
  \begin{itemize}
...
\begin{frame} {RNNs - Sentiment Analysis}
  \begin{itemize}
    ...
    \item $f = \tau (U^Tz^{[4]} + c)$, where $c$ is the bias of the output neuron.
  \end{itemize}
  \begin{figure}
      \centering
...
\begin{vbframe}{RNNS - Language Modelling}
  \begin{itemize}
    ...
    \item Output: probability distribution over words given previous words $$P(y^{[1]}, \dots, y^{[\tau]}) = \displaystyle P(y^{[1]}) \prod_{i=2}^{\tau} P(y^{[i]}|y^{[1]}, \dots, y^{[i-1]})$$
...
\begin{frame} {Word embeddings}
  \begin{itemize}
    \item In the preceding examples, words were encoded as sparse one-hot vectors.
...
\begin{frame} {Word embeddings}
  \begin{itemize}
    \item Two very popular sources of word embeddings are word2vec by Google and GloVe by Facebook. These embeddings are typically 100 to 1000 dimensional.
    \item Even though these embeddings capture the meaning of each word to an extent, they do not capture the \textit{context} of the word in a given sentence because each word has a static precomputed representation.
    \item For example, depending on the context, the word "bank" might refer to a financial institution or to a river bank.
    \item Recently, there have been significant breakthroughs in context-based embeddings. One such example is BERT [Devlin et al., 2018] which was trained on a corpus of 3.3 billion words.
    \item BERT obtained new state-of-the-art performance on 11 NLP tasks.
  \end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\frametitle{Backpropagation through time}
  ...
  \center
  \only<1>{\includegraphics[width=5cm]{plots/rnn_bppt1.png}}%
  \only<2>{\includegraphics[width=5cm]{plots/rnn_bppt3.png}}%
...
\frametitle{long short-term memory - LSTM}
  ...
  \begin{itemize}
    ...
    \only<2>{\item One such gate is the \textit{forget} gate. Think of a model trying to predict the next word based on all the previous ones. The cell state might include the gender of the present subject, so that the correct pronouns can be used. When we see a new subject, we want to forget the gender of the old one.}
...
%    $$f^{[t]} = \sigma(b^{(f)} + V^{(f)T} z^{[t-1]} + W^{(f)T} x^{[t]})$$
    ...
    \only<3>{\item $\sigma()$ is obviously a sigmoid, to squash the values to $[0,1]$}
    \only<4>{\item To compute the cell state $s^{[t]}$, the first step is to multiply (element-wise) the previous cell state $s^{[t-1]}$ by the forget gate $f^{[t]}$. $$f^{[t]} s^{[t-1]}, \text{ with } f^{[t]} \in [0,1]$$}
    \only<5>{\item Now it is time to add new information into the cell state using the \textit{input} gate $i^{[t]}$.}
    ...
    \only<6>{\item $\tilde{s}^{[t]} = tanh(b + V^T z^{[t-1]} + W^T x^{[t]}) \in [-1, 1]$}
    \only<6>{\item $i^{[t]} = \sigma(b_i + V_i^{T} z^{[t-1]} + W_i^{T} x^{[t]}) \in [0,1]$}
    \only<6>{\item $i$ here refers to "input" and $W_i$ and $V_i$ are the weights of the input gate.}
    ...
    \only<8>{\item First, we run a sigmoid layer which decides what parts of the cell state we're going to output: $q^{[t]} = \sigma(b_q + V_q^{T} z^{[t-1]} + W_q^{T} x^{[t]})$ }
    %\only<8>{\item }
...
\begin{vbframe}{RNN with output recurrence}
  \begin{figure}
    \includegraphics[width=5.5cm]{plots/out_rec.png}
  \end{figure}
  \begin{itemize}
...
\framebreak  
  \begin{figure}
    \includegraphics[width=5.5cm]{plots/rnn_bi.png}
...
\framebreak
  \begin{figure}
    \includegraphics[width=6.5cm]{plots/seq2seq_3.png}
    \caption{Attention at $t=t+1$}
  \end{figure}
\framebreak
  \begin{figure}
    \includegraphics[width=6.5cm]{plots/seq2seq_4.png}
