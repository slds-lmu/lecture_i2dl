...
\lecturechapter{9}{Generative Models}
...
\only<1>{
\begin{figure}
\hspace{-5cm}\includegraphics[width=5cm]{sample.png}
...
&=  \frac{1}{\sqrt{2\pi\sigma^2}} \exp\left(-\frac{1}{2}  \sum_{i=1}^{n} \frac{(x^{(i)}-\mu)^2}{\sigma^2}\right)\enspace.
\end{align*}
}
...
\only<1>{\includegraphics[width=7cm]{ImCrossO.pdf}}\only<2>{\includegraphics[width=7cm]{ImHeartR.pdf}}\only<3>{\includegraphics[width=7cm]{ImStarO.pdf}}\only<4>{\includegraphics[width=7cm]{ImCrossR.pdf}}\only<5>{\includegraphics[width=7cm]{ImStarB.pdf}}\only<6>{\includegraphics[width=7cm]{ImStarR.pdf}}
\end{center}}
\mode<handout>{\begin{center}
\includegraphics[width=7cm]{ImCrossO.pdf}
\end{center}}
...
\begin{center}
\includegraphics[width=5cm]{MyFigure8-8}
\end{center}
...
%\hspace{5.4cm}
\includegraphics[width=0.37\framewidth]{VAE.png}
...
Idea:
\begin{itemize}
\item Set $p_{\vec \theta}(\vec z)$ to something simple. % e.g. $\mathcal{N}(1,0)$.
...
\begin{itemize}
\item define the generativelmodel similar to VAEs,
   ...
   \item However, (under some assumptions!) optimizing the minimax loss is equivalent to implicitly minimizing a divergence measure.
    ...
    \begin{align*}
      JS(p_{\text{data}}|p) & =  \frac{1}{2} KL(p_{\text{data}}||\frac{p_{\text{data}}+p}{2}) + \frac{1}{2}  KL(p || \frac {p_{\text{data}} + p}{2}) \\
        KL(p_{\text{data}}|p) & = E_{p_{\text{data}}}[log \frac {p_{\text{data}}(\vec x)}{p(\vec x)}]
    \end{align*}
 % \vspace{2mm}
...
\begin{frame} {Non-Saturating Loss}
  ...
  \end{figure}
  \begin{itemize}
    \item Solution: Use a non-saturating generator loss instead:  $J^{(G)} = - \frac{1}{2} \E [\log D(G(\mathbf{x}))]$
