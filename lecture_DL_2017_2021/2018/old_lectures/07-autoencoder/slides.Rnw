...
\begin{frame} {Sparse Autoencoder}
  \begin{itemize}
    \vspace{4mm}
    \item We've seen that the "bottleneck" induced by the small number of neurons in the hidden layer in the undercomplete autoendoder forces it to learn the most important features needed for reconstruction.
    \vspace{4mm}
    \item If, on the other hand, the number of neurons in the hidden were equal to or greater than the dimension of the inputs, the network could simply \textit{memorize} the input and product a perfect reconstruction without really \textit{learning} anything.
    \vspace{4mm}
    \item Is it possible to force an autoencoder with a large number of neurons in the hidden layer to  discover interesting structure in the data?
      \begin{itemize}
        \item \textit{Yes}, if we impose a \textbf{sparsity constraint} on the hidden units
      \end{itemize}
  \end{itemize}
...
\begin{frame} {Sparse Autoencoder}
  \begin{itemize}
    \vspace{15mm}
    \item A hidden unit is considered to be "active" is its output value is close to 1 and "inactive" if its output value is close to 0 (sigmoid)
    \vspace{10mm}
    \item Our goal is to constrain the autoencoder in such a way that the activations of most hidden neurons are close to 0(for a given input $x^{(i)}$)
  \end{itemize}
\end{frame}
    
\begin{frame} {Sparse Autoencoder}
  \begin{itemize}
    \item Let $z_j(x^{(i)})$ denote the activation of the $j$-th hidden unit when the input is $x^{(i)}$
    \vspace{4mm}
    \item The average activation of hidden unit $j$ is
      \begin{equation*}
        \hat\rho_j = \frac {1}{n}\sum\limits_{i = 1}\limits^{n} z_j(x^{(i)})
      \end{equation*}
      where the average is over the samples in the training set.
    \vspace{4mm}
    \item We can then introduce the (soft) constraint 
      \begin{equation*}
        \hat\rho_j= \rho
      \end{equation*}
      where $\rho$ is \textbf{sparsity parameter} which is typically close to 0(for example, $\rho = 0.05$)
  \end{itemize}
\end{frame}

\begin{frame} {Sparse Autoencoder}
  \begin{itemize}
    \item In order to enforce the constraint, we choose the following penalty term
      \begin{equation*}
        \sum \limits_{j=1}\limits^m \rho \log{\frac {\rho}{\hat\rho_j}} + (1 - \rho) \log{\frac{1 - \rho}{1 - \hat\rho_j}}
      \end{equation*}
      where the sum is over all the hidden units.
      \vspace{4mm}
      \item This penalty term is based on the Kullback-Liebler divergence between a Bernoulli random variable with mean $\rho$ and a Bernoulli random variable with mean $\hat\rho_j$:
         \begin{equation*}
           KL(\rho \| \hat\rho_j) = \rho \log{\frac {\rho}{\hat\rho_j}} + (1 - \rho) \log{\frac{1 - \rho}{1 - \hat\rho_j}} 
         \end{equation*}
        \vspace{4mm}
        \item The KL divergence $KL(\rho \| \hat\rho_j) = 0$ if and only if $\hat\rho_j = \rho$ , and otherwise increases monotically as $\hat\rho_j$ diverges from $\rho$
  \end{itemize}
\end{frame}
...
\begin{frame} {Sparse Autoencoder}
    \begin{figure}
    \centering
      \scalebox{0.5}{\includegraphics{plots/kullback.png}}
      \caption{\footnotesize Because the KL divergence blows up to infinity as $\hat\rho_j$ approaches 0 or 1, the penalty term has the effect of causing $\hat\rho_j$ to be close to $\rho$ where it reaches a mininum of 0}
  \end{figure}
  \footnotesize{With the addition of the penatly term, the overall loss becomes
      \begin{equation*}
      L_{sparse}(x, dec(enc(x))) = \uauto + \beta \sum \limits_{j=1}\limits^m KL(\rho \| \hat\rho_j)
      \end{equation*}
...
\begin{frame} {Sparse Autoencoder}
  \begin{figure}
    \centering
      \scalebox{0.5}{\includegraphics{plots/sparse_auto.png}}
  \end{figure}
  \begin{itemize}
    \item Each square above shows the (norm bounded) input image that maximally activates one of 100 hidden units of a sparse autoencoder.
    \item Note : Each hidden unit has learned to detect an edge at a specific position and orientation
  \end{itemize}
\end{frame}

\begin{frame} {k-sparse autoencoder}
  \begin{itemize}
    \vspace{4mm}
    \item Yet another kind of sparse autoencoder is the \textbf{k-sparse autoencoder} \begin{footnotesize}(Makhzani, Frey 2014)\end{footnotesize}
    \vspace{4mm}
    \item It is based on an autoencoder with linear activation functions and tied weights.
    \vspace{4mm}
    \item In the feedforward phase, after the activations $\mathbf{z}$ are computed, rather than reconstructing the input from all of the hidden units, the $k$ largest hidden units(in terms of the activations) are identified and the rest are set to zero.
    \vspace{4mm}
    \item This selection step acts as a regularizer which prevents the use of an overly large number of hidden units when reconstructing the input.
  \end{itemize}
\end{frame}
...
\begin{frame} {k-sparse autoencoder}
  \begin{figure}
    \centering
      \scalebox{0.8}{\includegraphics{plots/ksparse_a.png}}
      \caption{\footnotesize {Filters of the $k$-sparse autoencoder for different sparsity levels $k$, learnt from MNIST with 1000 hidden units.For large values of k (e.g., k = 100), the algorithm tends to learn very local features}}
  \end{figure}
\end{frame}
...
\begin{frame} {k-sparse autoencoder}
  \begin{figure}
    \centering
      \scalebox{0.8}{\includegraphics{plots/ksparse_b.png}}
      \caption{\footnotesize {As the sparsity level is decreased (e.g., k = 25), the output is reconstructed using a smaller number of hidden units and thus the features tend to be more global}}
  \end{figure}
\end{frame}
