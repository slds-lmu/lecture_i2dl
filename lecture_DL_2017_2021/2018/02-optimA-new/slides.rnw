...
\begin{vbframe}{Gradient descent}
  \begin{itemize}
    \item Let $f(\mathbf{x})$ be an arbitrary, differentiable, unrestricted target function (of $\mathbf{x} \in \R^m$), which we want to minimize.
...
So far we have assmed a fixed value of $\alpha$ in every iteration :
...
\begin{footnotesize}
Steps of gradient descent for $f(\mathbf{x}) = 10 x_1^2 + 0.5 x_2^2$. Left :  100 steps for with a fixed learning rate. Right :  40 steps with an adaptive learning rate.
\end{footnotesize}
\end{center}
...
\begin{vbframe}{Chain rule of calculus}
  ...
  \begin{itemize}
    ...
    \item Let $\mathbf{x} \in \R^m$, $\mathbf{y} \in \R^n$, \\
          $g: \R^m \to \R^n$ and $f: \R^n \to \R$. \\
    \item If $\mathbf{y} = g(\mathbf{x})$ and $z = f(\mathbf{y})$, the chain rule yields : $$\frac{d z}{d x_i} = \sum_j \frac{d z}{d y_j} \frac{d y_j}{d x_i}$$
...
% \framebreak
  % \begin{itemize}
    ...
    Training of NNs happens in 2 consecutive steps, for one observation $x$:
      \begin{enumerate}
        ...
        \item Backward pass/Backpropagation: the information of the error that happened in the prediction of $x$ 
...
\begin{vbframe}{Weight update rule}
  \begin{itemize}
    \item Backpropagation can then be used to compute the gradient of $\Lxy$ in an \textit{extremely} efficient way.
...
\begin{vbframe}{Backpropagation example}
  \begin{itemize}
    ...
    \item For activations in both the hidden and output layers, we apply the \textbf{sigmoidal logistic function}.
    ...
    \item We optimize the model using the squared error between the binary 0-1 labels and the
      predicted probabilities rather than the Bernoulli loss - 
...
\begin{frame} {Summary}
  \begin{itemize}
      \item To perform gradient descent, we want to compute the gradient of the loss function $\risket$ with respect to \textit{all} the weights and biases in the network.
...
\begin{frame} {Summary}
  \begin{itemize}
    \item All the activations for the inputs that we feed to the network (forward pass) are stored in memory and used to compute the derivatives at each node in the computation graph. For example, to compute the derivative of the sigmoid activation at $z_{1,out}$, where $\frac{d z_{1,out}}{d z_{1,in}} = \sigma(z_{1,in})(1-\sigma(z_{1,in}))$ , we plugged in $\sigma(z_{1,in}) = \num[round-mode=places,round-precision=4]{0.3705169}$.
...
\begin{vbframe}{Stochastic gradient descent}
  \begin{itemize}
    \item Optimization algorithms that use the entire training set and update for them in one huge step are called \textbf{batch} or \textbf{deterministic}. This is computationally very costly, particularly in DL, or often impossible,
...
\framebreak
  ...
  \begin{itemize}
    \item As we divide the training set into minibatches, one per iteration and gradient update, an epoch has $\frac{n}{m}$ gradient updates.
