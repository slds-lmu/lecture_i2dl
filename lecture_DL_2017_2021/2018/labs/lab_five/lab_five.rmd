...
2. Stacking recurrent layers, to increase the representational power of the network (at the cost of higher computational loads).  
...
`data` -- The original array of floating-point data, which you just normalized.  
...
```{r}
# Build a fully connected model that starts by flattening the data and then runs it through two dense layers (First : 32 units, Relu. Second : 1 unit)
# Remember to specify the input_shape for the Flatten layer
...
# Train the model for 20 epochs with steps_per_epoch = 500
# Remember to pass validation data and set validations_steps = val_steps
...
#Build a model by replacing the Flatten and Dense(32) layers with a GRU layer (also with 32 units). Specify the input_shape.
#The output of the model will once again be a scalar(with no activation function)
model <- keras_model_sequential() %>% 
#
# your code here
#
  
# Compile and train the model with the same settings as the previous one.
...
```{r}
#Replace 'history' with whatever variable you sace
plot(history)
```
Much better! We are able to significantly beat the common sense baseline, demonstrating the value of machine learning here, as well as the superiority of recurrent networks compared to sequence-flattening dense networks on this type of task.

Our new validation MAE of ~0.265 (before we start significantly overfitting) translates to a mean absolute error of 2.35 degrees C after de-normalization. That's a solid gain on our initial error of 2.57 degrees C, but we probably still have a bit of margin for improvement.
...
#Retrain the model(from scratch) with dropout = 0.2 and recurrent_dropout = 0.2
model <- keras_model_sequential() %>% 
#
# your code here
#

# Compile and train the model with (almost) the same settings as the previous one. Remember to double the number of epochs to 40.
#
# Your code here (Compile)
#
...
```{r}
# Build a neural net with the following architecture
# First layer : GRU (32 units, dropout of 0.1, recurrent dropout of 0.5). Specify input_shape.
# Second layer : GRU(64 units, ReLU activation,  dropout of 0.1, recurrent dropout of 0.5)
# Final layer : Scalar output with no activation function
model <- keras_model_sequential() %>% 
#
# your code here
#
...
#Retrain(from scratch) model 5A above on the reverse order data.
#Compile and fit the model with the exact same settings.
...
# Reverse sequences in x_train and y_train
#
# your code here
#

# Pad sequences in x_train and y_train so they're the same length(that is, maxlen)
#
# your code here
#

# Use the same overall architecture as model 4A but increase the dimensionality of the embeddings to 128
# Compile and train it(with the exact same settings) on the reverse order data 
...
# Modify the architecture in 4A by converting the LSTM layer to a bidirectional LSTM layer
# The dimensionality of the embeddings remains the same (32)
# Compile and train it with the exact same settings (as 4A)
# Note : You can feed the reverse order data to fit()
...
```{r}
# Modify the architecture in 5A by converting the GRU layer to a bidirectional GRU layer
# Compile and train for 40 epochs (rather than 20) with the same settings as 5A
# Note : Feed the chronological order data to fit_generator()
