...
\begin{frame} {Introduction}
 All machine learning algorithms consist of three key components:
  ...
  %  \vspace{4mm}
    ...
    \begin{itemize}
      \item A metric (\textbf{loss/objective function}) by which to evaluate models in the hypothesis space.
...
\begin{frame} {A Single Neuron : Evaluation}
  \begin{itemize}
    \item   
...
%    \item For a single neuron, in both cases, the loss function is convex and the global optimum can be found with an iterative algorithm like gradient descent. \small{(Note : In the case of regression, the solution can also be found analytically using the 'normal equations'.)}
 \item In both cases, the loss function is convex and thus has a global optimum \small{(Recall : In the case of regression, the solution can be found analytically using the 'normal equations'.)}
  \end{itemize}
\end{frame}  
...
\begin{frame} {Neural Networks}
  \begin{itemize}
    ...
    \vspace{5mm}
    \item %It's useful because 
    This visual metaphor allows us to use such individual neurons as building blocks 
     of networks which represent considerably more complicated functions, and thus more complex hypothesis spaces.
    \vspace{5mm}
   % \item %Therefore, 
   % Networks of neurons can represent extremely complex hypothesis spaces.
   % \vspace{5mm}
    \item %Most importantly, 
    It allows us to define the "right" kinds of hypothesis spaces to learn functions that are more common in our universe in a data-efficient way (see Lin, Tegmark et al. 2016).
  \end{itemize}
\end{frame}
...
\begin{frame} {Neural Nets : Representation Learning}
  \begin{itemize}
    \vspace{5mm}
    \item It is \textit{very} critical to feed a classifier the "right" features in order for it to perform well.
    \vspace{7mm}
    \item Before deep learning (DL) took off, features for tasks like machine vision and speech recognition were "hand-designed" by domain experts.This step of the machine learning pipeline is called \textbf{feature engineering}.
    \vspace{7mm}
    \item The %single
     biggest reason DL is so important is that it automates feature engineering.This is called \textbf{representation learning}.
  \end{itemize}
\end{frame}
...
\begin{frame} {Neural Nets : Representation Learning}
  \begin{itemize}
    ...
    \vspace{3mm}
    \item The final neuron %will now be 
    is referred to as the \textbf{output neuron}.
    \vspace{3mm}
...
\framebreak
  ...
  \begin{itemize}
    ...
    \item For regression or binary classification: one output unit $f$, where,
      \begin{itemize}
        \item $f_{in} = \wtu^T \hidz + c$ 
          \begin{itemize}
            \item This is a linear combination of derived features.
            \item $c$ is the bias of the output neuron.
          \end{itemize}
        \item $f(x) = f_{out} = \tau(f_{in}) = \tau(\wtu^T \hidz + c)$ , where $\tau$ is the output activation.
      \end{itemize}
      \vspace{4mm}
  ...
  \begin{itemize}
    ...
    \item The weighted sum in the hidden layer is now computed as $XW + B$, where,
      \begin{itemize}
        ...
        \vspace{2mm}
        \item $B$ is a ($n \times m$) matrix containing the bias vector $\biasb$ ($n$ copies!) as the rows of the matrix.
      \end{itemize}
    \vspace{4mm}
...
\begin{frame} {Hidden Layer : Activation Function}
  \begin{itemize}
    \item Claim: if the hidden layer doesn't have a non-linear activation (i.e. $\sigma$ is the identity function), the network can only learn linear decision boundaries.
    \item Proof: To drop the bias term in the notation, let's add a feature $x_0=1$ to $\mathbf{x}$ %which always takes the value '1'. 
      %\begin{itemize}
       % \item 
       $$\tilde{\mathbf{x}} = (x_0, x_1, \ldots, x_p)^T =(1,  x_1 \ldots, x_p)^T$$
     % \end{itemize}
    \item  and let $\tilde{W}$ %is now  denote
    contain weight matrix W and bias $\biasb$ in an additional first row.
      \begin{equation*}. 
        \begin{split}
        f & = \tau(\wtu^T \hidz + c) \\
...
%\vspace{4mm}
 \footnotesize{%Note : 
 Lets compute all examples at once. X is a $(n \times p)$ design matrix in which the \textit{rows} correspond to the data points. W, as usual, is a $(p \times m)$ matrix where each \textit{column} corresponds to a single (hidden) neuron. B is a ($n \times m$) matrix with $\biasb$ duplicated along the rows.}
 \begin{figure}
    \centering
...
\framebreak
  \begin{minipage}{0.45\textwidth}
    \begin{itemize}
      \item Input points are mapped in transformed space to
        \begin{eqnarray*}
          Z = \begin{pmatrix}
              0 & 0 \\
              1 & 0 \\
    ...
    \begin{itemize}
      \item Input points are mapped in transformed space to
        \begin{eqnarray*}
          ...
          \end{pmatrix}
        \end{eqnarray*}
      \item which are easily separable.
    \end{itemize}
  \end{minipage}
  \begin{minipage}{0.5\textwidth}
...
\framebreak
  \begin{itemize}
    \item In a final step we %multiply the 
    %states of the hidden units with the weights of the output layer
   first calculate the input of  the output neuron 
    :%activated values of matrix Z with the vector u:
  \end{itemize}
  \begin{eqnarray*}
  ...
  \end{eqnarray*}
  \begin{itemize}
    \item And then apply a step-function (with 0 threshold) as activation function. This solves the XOR problem perfectly!
  \end{itemize}
  \begin{table}
    \centering
      \begin{tabular}{ccc}
...
\subsection{Artificial Neural Networks for Multi-Class Classification}
...
\begin{vbframe}{Deep feedforward networks}
  \begin{itemize}
    \vspace{15mm}
    \item We will now extend the model class once again, such that we allow an arbitrary amount of hidden layers.
    \vspace{5mm}
...
\vspace{-2cm}
  \begin{figure}
    \centering
      \includegraphics[width=5cm]{plots/hierachicalRepresentations.png}
