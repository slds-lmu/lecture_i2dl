...
batchnorm1 = mx.symbol.BatchNorm(data = act1)
pool1 = mx.symbol.Pooling(data = batchnorm1, pool_type = "max", kernel = c(2, 2), stride = c(2, 2))
# second conv
conv2 = mx.symbol.Convolution(data = pool1,  kernel = c(3, 3), num_filter = 30)
act2 = mx.symbol.Activation(data = conv2, act_type = "relu")
batchnorm2 = mx.symbol.BatchNorm(data = act2)
pool2 = mx.symbol.Pooling(data = batchnorm2, pool_type = "max", kernel = c(2, 2), stride = c(2, 2))
...
fc2 = mx.symbol.FullyConnected(data = dropout1, num_hidden = 128)
