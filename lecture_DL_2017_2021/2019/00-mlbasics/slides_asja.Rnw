...
\lecturechapter{0}{\textcolor{black}{Introduction to Machine Learning I}}
...
A \textbf{model} (or \textbf{hypothesis}) %$f : \Xspace \rightarrow \R^g$ 
...
Nearly all ML supervised %learning 
training algorithms can be described
by three components:
...
\item
 ...
 structures of models / hypothesises \(f\) we can learn.
\item
...
%  structures of models/hypothesises \(f\) we can learn.
%\item
%  \textbf{Evaluation:}  Quantification of the perfromance of a hypothesis %score 
%  given a data set.  Allows us to choose better candidates over worse ones.
%\item
%  \textbf{Optimization:} how do we search the hypothesis space? Guided
%  by the evaluation metric.
\end{itemize}
...
Let \(\mathbb P_{xy}\) be the joint distribution of \(x\) and \(y\), where $\D$ is i.i.d sampled from.
  It defines all aspects of the generating process where our data
  comes from. 
\vfill
The quality of the prediction $y=\fx$ of a model is measured by a \textbf{loss function} $\Lxy $.
\vfill
This is a  function $L :\Yspace \times \Yspace \rightarrow [0, \infty[$ with the property $L(y, y) = 0$ for $y \in  \Yspace$. %is called a loss function.
\vfill
%A typical loss function is the squared error:
...
\item We assume %here and 
from now on that \(\theta_0\) is included in
...
We want to find the parameters of the linear model / an element of the hypothesis
...
$$
\LLt = \prod_{i=1}^n   \pdf\Big(\yi | \xi , \theta \Big)  %\pdf(\yi | \fxit, \sigma^2) 
...
An %\emph{discriminant} 
approach for directly modeling the posterior probabilities of the classes is \textbf{logistic regression}. We will first look at the binary case $y \in \mathcal{Y}=\{0, 1\}$ and define our model as
...
evaluated based on the \textbf{(log-)likelihood}.
\begin{eqnarray*}
...
\begin{eqnarray*}
\llt
&=& \sum_{i=1}^n \yi \log[ P(y = 1 | x^{(i)}, \theta)^{y^{(i)}}] + (1-\yi) \log [1 - P(y = 1 | x^{(i)}, \theta)^{y^{(i)}}]
...
%Todo: adapt formula!!!
%The \textbf{cross-entropy loss} for a  classifier $\pi(\xi, \theta)$  is given by 
%$$
%-\llt= -\lpdfygxt = 
%- y \log[\pi(\xi, \theta)] - (1-y) \log [1 - \pi(\xi, \theta)]
%$$
% %is the \textbf{cross-entropy loss}. 
%\normalsize
...
\item an appropriate loss is needed, e.g. %binomial or 
...
%binary 
score model $f_k(x)$ (that replaces $\theta_k^Tx$) with parameter vector $\theta_k$.
We then combine these models through the softmax function

$$
P(y=k|x) = \pikx = % s_k(f_1(x), \ldots f_g(x))
s(f_1(x), \ldots, f_g(x))_k
$$

and optimize all parameter vectors %of the $f_k$ 
