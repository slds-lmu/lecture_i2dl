...
\newcommand{\Oreg}{\mathcal{R}_{reg}(\theta|X,y)}				% regularized objective function
...
\newcommand{\Lxym}{L(\yi, f(\tilde{x}^{(i)}, \theta))} 
\newcommand{\Lmomentum}{L(\yi, f(\tilde{x}^{(i)}, \theta + \varphi \nu))}                  % momentum risk

% deeplearning - autoencoders
\newcommand{\uauto}{L(x,dec(enc(x)))}			% undercomplete autoencoder objective function
\newcommand{\dauto}{L(x,dec(enc(\tilde{x})))}		% denoising autoencoder objective function
