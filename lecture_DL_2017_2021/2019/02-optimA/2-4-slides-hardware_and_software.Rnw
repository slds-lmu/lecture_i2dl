...
\begin{frame} {Graphics Processing Units (GPUs)}
  \begin{itemize}
    ...
    \item Commonly used: GTX 1080 Ti, RTX 2080, Titan X/RTX, Tesla V100, Tesla K80
...
\begin{frame} {Tensor Processing Units (TPUs)}
  \begin{itemize}
    ...
    \item Not a consumer product! Can be used in the Google Cloud Platform (8 USD / TPU / hour) or Google Colab (free! Python only)
    \item Enables DeepMind to do ridiculous things: AlphaZero for Chess became world champion after just 4 hours of training concurrently on 5064 TPUs
  \end{itemize}
\end{frame}
...
\textbf{Tensorflow}
\begin{itemize}
  \item Very popular in the industry
...
\textbf{Keras}
  \begin{itemize}
    ...
    \item Capable of running on Tensorflow, CNTK or Theano backends (will only support TF in the near future)
...
\textbf{Pytorch}
  \begin{itemize}
    \item Booming in popularity in academia this year
