...
\begin{frame} {A Single Neuron}
  \begin{itemize}
    ...
    \item It is very straightforward to represent this function $\fx$ graphically as a neuron.
...
\begin{vbframe} {A Single Neuron: Optimization}
  \begin{itemize}
    ...
    \item For binary classification, we typically apply the cross entropy loss (also known as binomial loss): 
    ...
    \item Note: In the case of regression and the L2-loss, the solution can also be found analytically using the \enquote{normal equations}. However, a closed-form solution is usually not available. 
  \end{itemize}
\end{vbframe} 
