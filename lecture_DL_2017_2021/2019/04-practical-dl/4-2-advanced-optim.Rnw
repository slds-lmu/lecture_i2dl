...
\begin{vbframe}{Momentum}
  \begin{itemize}
    ...
    %\item As shown on the following slide, we can think of momentum as the fraction $\frac{1}{1-\varphi}$. Then, $\varphi = 0.9$ corresponds to multiplying the maximum speed by 10 relative to
...
E.g. a momentum with $\varphi = 0.9$ corresponds to multiplying the maximum speed by 10 relative to
the gradient descent algorithm. 
\end{vbframe}
...
\begin{algorithm}[H]
  ...
  \State Initialize 1st and 2nd moment variables $\mathbf{s}^{[0]} = 0, \mathbf{r}^{[0]} = 0$
    \While{stopping criterion not met}
      ...
      \State Correct bias in first moment: $\hat{\mathbf{s}}^{[t]} \leftarrow \frac{\mathbf{s}^{[t]} }{1-\rho_1^t}$
      \State Correct bias in second moment: $\hat{\mathbf{r}}^{[t]} \leftarrow \frac{\mathbf{r}^{[t]} }{1-\rho_2^t}$
...
%   \begin{itemize}
%   \small
%     \item Let us also try out different optimizers on the standard architecture for the mnist data set.  
%     \item The optimizers are: 
%     \begin{itemize}
%       \small
%       \item \textbf{Adagrad} with a learning rate of 0.05 and a $\beta$ of $10^{-8}$.
%       \item \textbf{Adam} with a learning rate of 0.001 and decay rates of $\rho_1 = 0.9$ and $\rho_2 = 0.999$, as well as a $\beta$ of $10^{-8}$.
%       \item \textbf{RMSProp} with a global learning rate of 0.002 and a decay rate $\rho$ of 0.95. 
%       \item \textbf{SGD with momentum} with a learning rate of 0.03 and a momentum of 0.9.
%     \end{itemize}
%     \item Each optimizer uses a weight decay of 0.001. 
%     \end{itemize}
% <<mxnet2, size = "scriptsize", cache = TRUE, eval = FALSE, echo = FALSE>>=
