...
\lecturechapter{10}{Recurrent Neural Networks (RNNs)}
...
\begin{frame} {Application example - Sentiment Analysis}
  \begin{itemize}
    ...
    \item $\bm{z}^{[2]} = \sigma(\bm{V}^\top\textcolor{red}{\bm{z}^{[1]}} + \bm{V}^\top \xv^{[2]} + \bm{b})$
  \end{itemize}
  \begin{figure}
      \centering
...
\begin{vbframe}{Long-Term Dependencies}
  ...
  \begin{itemize}
    ...
    $$ \frac{d \bm{z}^{[t]}}{d\bm{z}^{[t-1]}} = \text{diag}(\sigma' (\bm{V}^\top \bm{z}^{[t-1]} + \bm{W}^\top \xv^{[t]} + \bm{b})) \bm{V}^\top$$
