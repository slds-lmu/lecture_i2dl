...
\begin{vbframe}{Reset Gates in Action}
     \begin{figure}
      \centering
      \scalebox{0.5}{\includegraphics{plots/gru_2.png}}
  ...
  \item Figure 2 illustrates the computational flow after applying the reset gate. 
  \end{itemize}
...
\begin{vbframe}{Update Gates in Action}
  ...
  \end{itemize}
   \begin{figure}
      \centering
      \scalebox{0.5}{\includegraphics{plots/gru_3.png}}
      \caption{\footnotesize{Hidden state computation in a GRU. As before, the multiplication is carried out elementwise.}}
  \end{figure}
  
  These designs can help us cope with the vanishing gradient problem in RNNs and better capture dependencies for time series with large time step distances. In summary, GRUs have the following two distinguishing features:
  \begin{itemize}
