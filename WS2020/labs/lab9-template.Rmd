...
train_logreg = function(data_x, data_y, max_steps, lrate, batch_size) {
  ...
  losses = sapply(1:(max_steps + 1), function(i) {
    ...
    #!hwbegin TODO perform the forward pass, compute predictions and loss
    linear = batch_x %*% theta
    loss = sum(log(1 + exp(-batch_y * linear[,1]))) / batch_size
    #!hwend
    
    #!hwbegin TODO perform the backward pass, compute the gradient of theta
    es = exp(-batch_y * linear[,1])
    grad = -batch_y * es / (1 + es) / batch_size
    grad = colSums(sweep(batch_x, MARGIN = 1, grad, '*'))
    #!hwend
    
    #!hwbegin TODO update theta with one step of gradient descent
...
predict_class = function(x) {
  #!hwbegin TODO compute the class of x (+1 or -1)
...
perturb_and_check = function(eps) {
  delta = (
    #!hwbegin TODO create a random vector with elements +/- eps
    eps * sign(rnorm(length(sample)))
    #!hwend
  )
  
  prediction = (
    #!hwbegin TODO perturb the sample and predict its class
...
make_adversarial = function(theta, sample, true_class, eps, max_steps, lrate) {
  ...
  losses = sapply(1:(max_steps + 1), function(i) {
    ...
    #!hwbegin TODO perturb the sample with delta and compute the linear part of the model
    ...
    #!hwbegin TODO compute the gradient of delta with respect to loss_true 
    est = exp(-true_class * linear[,1])
    grad_true = -true_class * theta * est / (1 + est)
    #!hwend
    
    #!hwbegin TODO compute the gradient of delta with respect to loss_wanted
    esw = exp(-wanted_class * linear[,1])
    grad_wanted = -wanted_class * theta * esw / (1 + esw)
    #!hwend
    
    #!hwbegin TODO update delta with one step of gradient ascent
    delta <<- delta + lrate * (grad_true - grad_wanted)
    #!hwend

    #!hwbegin TODO normalize delta to have elements +/- eps
