...
\underbrace{
  \frac{\partial\mathcal{L}(\textbf{y},\textbf{z}^{(L)}_{\cdot,out})}{\partial\textbf{z}^{(L)}_{i,out}}
}_{\text{Loss-dependent}}\cdot{\sigma^\prime}^{(\ell)}(\textbf{z}^{(\ell)}_{i,in})
\end{align}
...
\odot{\sigma^\prime}^{(L)}(\textbf{z}^{(L)}_{\cdot,out}) \\
...
\odot{\sigma^\prime}^{(\ell-1)}(\textbf{z}^{(\ell-1)}_{\cdot,out}) \\
(N^{(\ell-1)}\times 1)&=\left(
...
\odot{\sigma^\prime}^{(L)}(\textbf{Z}^{(L)}_{out})
\end{equation}
...
\right)\cdot{\sigma^\prime}^{(\ell-1)}(\textbf{Z}^{(\ell-1)}_{ij,out})
\end{equation}
...
nnet.gradients = function(nnet, x, y) {
  ...
  ) * nnet$activations_derivatives[[n_layers]](activations[[length(activations)]])
  for(l in n_layers:1) {
    ...
    if(l > 1) {
      ...
      deltas = deltas %*% t(
        nnet$weights[[l]]
      ) * nnet$activations_derivatives[[l - 1]](activations[[l]])
    }
  }
