...
```{r}
original_model = (
  #!hwbegin TODO create the network according to the specifications above
...
```{r}
smaller_model = (
  #!hwbegin TODO create a smaller model according to the specifications above
  keras_model_sequential() %>% 
...
```{r}
original_hist = fit(
  #!hwbegin TODO fit the original model
  original_model,
  x_train, y_train,
  batch_size = 512,
  epochs = 20,
...
smaller_model_hist = fit(
  #!hwbegin TODO fit the smaller model
  smaller_model,
  x_train, y_train,
  batch_size = 512,
  epochs = 20,
...
```{r}
bigger_model = (
  #!hwbegin TODO build a network with 512 units in the two hidden layers
  keras_model_sequential() %>% 
...
compile(
  #!hwbegin TODO compile this network with the same settings we used previously
  bigger_model,
  optimizer = "rmsprop",
  loss = "binary_crossentropy",
  metrics = c("accuracy")
...
```{r}
l2_model = (
  #!hwbegin TODO modify the original model to have L2 regularization with coefficient 0.001
  keras_model_sequential() %>%
    layer_dense(units = 16, activation = "relu", input_shape = c(10000),
...
compile(
  #!hwbegin TODO compile the model with the usual configuration
  l2_model,
  optimizer = "rmsprop",
  loss = "binary_crossentropy",
  metrics = c("accuracy")
...
```{r}
plot_training_losses(list(
  #!hwbegin TODO plot the validation losses
  orig=original_hist$metrics$val_loss,
  regu=l2_model_hist$metrics$val_loss
  #!hwend
))
```
...
```{r}
dpt_model = (
  #!hwbegin TODO create a copy of the original model using dropout with probability 0.5
  keras_model_sequential() %>% 
    layer_dense(units = 16, activation = "relu", input_shape = c(10000)) %>% 
    layer_dropout(0.5) %>%
    layer_dense(units = 16, activation = "relu") %>% 
...
```{r}
dpt_model_hist =  fit(
  #!hwbegin TODO train the dropout-regularized model with the usual settings
  dpt_model,
  x_train, y_train,
  batch_size = 512,
  epochs = 20,
...
```{r}
plot_training_losses(list(
  #!hwbegin TODO plot the validation losses of dpt_model and original_model
  orig=original_hist$metrics$val_loss,
...
dpt_model_early_hist = fit(
  ...
  validation_data = list(x_test, y_test),
  callbacks = list(
    #!hwbegin TODO use the early stopping callback
...
```{r}
#!hwbegin TODO compute and print the loss on the test samples
dpt_model_early %>% evaluate(x_test, y_test, verbose = 0)
#!hwend
```
...
To find the eigenvalues, solve $\text{det}(\textbf{H}-\beta_i\textbf{I})=0$, i.e. $(\lambda_1-\beta_1)(\lambda_2-\beta-2)=0$, which clearly shows that the eigenvalues are $\lambda_1$ and $\lambda_2$. To find the corresponding eigenvectors, solve $\textbf{Hx}=\lambda_1\textbf{x}$, which gives:
...
Hence, smallest rate of convergence occurs for $\lambda_i=\lambda_{\text{min}}$.
...
make_gradient = function(lambda1, lambda2) {
  function(p) c(
  #!hwbegin TODO compute the two components of the gradient of E at p
...
gradient_descent_optimizer = function(x0, max_steps, lrate) {
  ...
  sapply(1:(max_steps + 1), function(i) {
    old = point
    #!hwbegin TODO modify `point` performing one step of gradient descent\n# remember to use the '<<-' operator to assign the new value to `point`
...
```{r}
max_learning_rate = (
  #!hwbegin TODO compute the maximum learning rate possible in this case
...
max_learning_rate = (
  #!hwbegin TODO compute the maximum learning rate
  2 / 30
  #!hwend
)
...
momentum = (
  #!hwbegin TODO try several values for the momentum
  0.5
  #!hwend
)
