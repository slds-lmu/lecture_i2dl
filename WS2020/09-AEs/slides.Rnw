...
\begin{vbframe}{Very powerful AE -- problem}
...
\begin{vbframe}{Learning Manifolds with AEs}
    ...
    \begin{itemize}
        \item AE training procedures involve a compromise between two forces:
...
\begin{frame}{Experiment: Encode MNIST with a DAE}
  \begin{itemize}
    \item We will now corrupt the MNIST data with Uniform noise and then try to denoise it as good as possible.
  \end{itemize}
  \begin{figure}
    \centering
...
\begin{figure}
    \centering
    % \includegraphics[width=6cm]{plots/unsupervised_2.png}
    \scalebox{1}{\includegraphics{plots/dae_code.png}}
\end{figure}
...
\textbf{Latent space traversal}:
\begin{itemize}
  \item Once an AE has been trained to compress images, it's possible to "generate" new images by feeding arbitrary latent vectors $\mathbf{z}$ to the decoder.
  \item In fact, certain axes/directions in the latent space can correspond to interpretable factors of variation in the images generated.
  \item In this example, the decoder was trained to map latent code to high-school yearbook photos and interesting axes were discovered by performing PCA on the latent code of the training data.
  \item \href{https://www.youtube.com/watch?v=4VAkrUNLKSo}{Click here} for a video of the experiment.
\end{itemize}
\framebreak
\textbf{Latent space traversal}:
\begin{figure}
    \centering
    % \includegraphics[width=6cm]{plots/unsupervised_2.png}
    \scalebox{1}{\includegraphics{plots/ae_yb2.png}}
  \end{figure}
...
\begin{figure}
    \centering
    % \includegraphics[width=6cm]{plots/unsupervised_2.png}
    \scalebox{1}{\includegraphics{plots/ae_yb3.png}}
  \end{figure}
