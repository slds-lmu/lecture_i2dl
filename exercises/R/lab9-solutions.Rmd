...
#TODO
\begin{align}
\nabla_{\textbf{h} ^{[\tau]}}L = \bigg(  \frac{\partial\textbf{y}^{{[\tau]}}}{\partial\textbf{h}^{{[\tau]}}}\bigg)^{T} (\nabla_{\textbf{y}{^{[\tau]}}}{{L}}) = \textbf{V}^{T} (\nabla_{\textbf{y} ^{[\tau]}}L)
\end{align}

\begin{align}
 \nabla_{\textbf{h} ^{[t]}}L = \bigg(  \frac{\partial\textbf{h}^{{[t+1]}}}{\partial\textbf{h}^{{[t]}}}\bigg)^{T} (\nabla_{\textbf{h}{^{[t+1]}}}{{L}}) +  \bigg(  \frac{\partial\textbf{y}^{{[t]}}}{\partial\textbf{h}^{{[t]}}}\bigg)^{T} (\nabla_{\textbf{y}{^{[t]}}}{{L}})
= \textbf{W}^{T} diag\bigg(1-\big(\textbf{h}^{[t+1]}\big)^{^2} \bigg)(\nabla_{\textbf{h}{^{[t+1]}}}{{L}}) + \textbf{V}^{T} (\nabla_{\textbf{y} ^{[t]}}L)
\end{align}
#TODO ends
...
\begin{align}
\nabla_\textbf{c} L = \sum_t\bigg(  \frac{\partial\textbf{y}^{{[t]}}}{\partial\textbf{c}}\bigg)^{T} \nabla_{\textbf{y}{^{[t]}}}{{L}} = \sum_t\nabla_{\textbf{y}{^{[t]}}}{{L}}
\end{align}
\begin{align}
\nabla_\textbf{b} L = \sum_t\bigg(  \frac{\partial\textbf{h}^{{[t]}}}{\partial\textbf{b}}\bigg)^{T} \nabla_{\textbf{h}{^{[t]}}}{{L}} = \sum_t diag\bigg(1-\big(\textbf{h}^{[t]}\big)^{2} \bigg) \nabla_{\textbf{h}{^{[t]}}}{{L}}
\end{align}

\begin{align}
\nabla_\textbf{V} L = \sum_t\sum_i\bigg(  \frac{\partial L}{\partial y_i^{[t]}}\bigg)  
\nabla_{\textbf{V}^{[t]}} y_i^{[t]} = \sum_t(\nabla_{\textbf{y}{^{[t]}}}{{L}})  \textbf{h}^{{[t]}^{T}}
\end{align}

\begin{align}
\nabla_\textbf{W} L = \sum_t\sum_i\bigg(  \frac{\partial L}{\partial h_i^{[t]}}\bigg)  
\nabla_{\textbf{W}} h_i^{[t]} = \sum_t diag\bigg(1-\big(\textbf{h}^{[t]}\big)^{2} \bigg)\ (\nabla_{\textbf{h}{^{[t]}}}{{L}}) \textbf{h}^{{[t-1]}^{T}}
\end{align}

\begin{align}
\nabla_\textbf{U} L = \sum_t\sum_i\bigg(  \frac{\partial L}{\partial h_i^{[t]}}\bigg)  
\nabla_{\textbf{U}} h_i^{[t]}  = \sum_t diag\bigg(1-\big(\textbf{h}^{[t]}\big)^{2} \bigg)(\nabla_{\textbf{h}{^{[t]}}}{{L}}) \textbf{x}^{{[t]}^{T}}
\end{align}
...
\begin{align}
\nabla_{\textbf{h} ^{[\tau]}}L = \textbf{V}^{T} (\nabla_{\textbf{y} ^{[\tau]}}L) = \textbf{V}^{T} \frac{\partial L} {\partial y} = 2(\hat y-y) \textbf{V}^{T}
\end{align}
\begin{align}
\nabla_{\textbf{h} ^{[t]}}L = \bigg(  \frac{\partial\textbf{h}^{{[t+1]}}}{\partial\textbf{h}^{{[t]}}}\bigg)^{T} (\nabla_{\textbf{h}{^{[t+1]}}}{{L}}) = \textbf{W}^{T} diag\bigg(1-\big(\textbf{h}^{[t+1]}\big)^{2} \bigg)(\nabla_{\textbf{h}{^{[t+1]}}}{{L}})
\end{align}

\begin{align}
(\nabla_{c} L) =  \sum_t\nabla_{\textbf{y}{^{[t]}}}{{L}} =  \nabla_{\textbf{y}{^{[\tau]}}}{{L}} = \frac{\partial L} {\partial y} = 2(\hat y-y)
\end{align}

\begin{align}
(\nabla_\textbf{V} L) = \sum_t(\nabla_{\textbf{y}{^{[t]}}}{{L}})  \textbf{h}^{{[t]}^{T}} = 2(\hat y-y) \textbf{h}^{{[\tau]}^{T}}
\end{align}
#TODO ends
...
backward_V <- function(Y_est, Y, last_H){
    ...
    for (i in 1:(dim(H)[1])){
        # Compute the parameter gradients and accumulate the results.
        # write code here
        V_grad <- V_grad + 2 * (Y_est[i] - Y[i]) * array(last_H[i,],dim = c(1,dim(last_H)[2]))
        # end of code
    }
    return (V_grad)
}   
