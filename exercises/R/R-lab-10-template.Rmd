...
We want to use 4 attention heads, each with a dimension of 64. The input to the attention layer is the output of the embedding layer. The output of the attention layer is then averaged over the time dimension (the second one) and passed to a dense layer with a single output.

```{r}
input <- layer_input(shape = c(NULL), dtype = "int32")

model <- keras_model_sequential |>
  layer_embedding(input_dim = 10001, output_dim = 64, mask_zero = TRUE) |>
  #!hwbegin TODO Add the multi-head attention layer and the average pooling layer
  layer_multi_head_attention(num_heads = 4, key_dim = 64)(query = embedding, key = embedding, value = embedding) |>
