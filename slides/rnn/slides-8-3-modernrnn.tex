...
\only<1>{\item Each gate is a vector of the same size as the $\bm{s}^{[t]}$ with elements between 0 ("let nothing pass") and 1 ("let everything pass").
    ...
    \only<2>{\item \textbf{Forget gate}  $\bm{e}^{[t]}$: indicates which information of the old cell state we should forget. 
...
%    $$f^{[t]} = \sigma(b^{(f)} + V^{(f)T} z^{[t-1]} + W^{(f)T} x^{[t]})$$
    $$\bm{e}^{[t]} = \sigma(\bm{b}_{f} + \bm{V}_e^\top \bm{z}^{[t-1]} + \bm{W}_e^\top \xv^{[t]})$$}
    \only<3>{\item $\sigma()$ is  a sigmoid, squashing the values to $[0,1]$, and $\bm{V}_e, \bm{W}_f$ are forget gate specific weights.}
    \only<4>{\item To compute the cell state $\bm{s}^{[t]}$, the first step is to multiply (element-wise) the previous cell state $\bm{s}^{[t-1]}$ by the forget gate $\bm{e}^{[t]}$. $$\bm{e}^{[t]} \odot \bm{s}^{[t-1]}, \text{ with } \bm{e}^{[t]} \in [0,1]$$}
   ...
   \only<6>{\item $\bm{W}$ and $\bm{V}$ are weights of the new information, $\bm{W}_i$ and $\bm{V}_i$ the weights of the input gate.}
    ...
    $$\bm{s}^{[t]} = \bm{e}^{[t]} \odot \bm{s}^{[t-1]} + \bm{i}^{[t]} \odot \tilde{\bm{s}}^{[t]}$$}
   ...
   \textbf{Output gate} $\bm{o}^{[t]}$:  Indicates which information form the cell state is filtered.
   \item It is given by $\bm{o}^{[t]} = \sigma(\bm{b}_o + \bm{V}_o^\top \bm{z}^{[t-1]} + \bm{W}_o^\top \xv^{[t]})$, with specific weights $\bm{W}_o, \bm{V}_o$.}
    %\only<8>{\item }
    \only<9>{\item Finally, the new state $\bm{z}^{[t]}$ of the LSTM is a function of the cell state, multiplied by the output gate: $$\bm{z}^{[t]} = \bm{o}^{[t]} \odot \text{tanh}(\bm{s}^{[t]})$$}
...
\frametitle{Gated Recurrent Units (GRU)}
   ...
   \begin{itemize}
     ...
     \begin{itemize}
           ...
           \item GRUs are able to solve the vanishing gradient problem by using an update gate and a reset gate. 
