...
# second conv
conv2 = mx.symbol.Convolution(data = pool1,  kernel = c(5, 5), num_filter = 128)
...
dropout1 = mx.symbol.Dropout(data = act3, p = 0.4)
fc2 = mx.symbol.FullyConnected(data = dropout1, num_hidden = 1024)
act4 = mx.symbol.Activation(data = fc2, act_type = "relu")
dropout2 = mx.symbol.Dropout(data = act4, p = 0.2)
...
write.csv(cnnError, file = "cnnError_deep", row.names = FALSE, quote = FALSE)

cnnError = read.csv("cnnError_deep", header = TRUE)
options(scipen = 999)
cnnError$variable = factor(cnnError$variable)

ggplot(data = cnnError, aes(x = epoch, y = value, colour = variable)) +
  geom_line() +
  scale_y_continuous(name = "misclassification", limits = c(0, 0.03), 
                     breaks=c(0, 0.0067, 0.01, 0.02, 0.03)) + 
  scale_x_continuous(labels = function (x) floor(x), 
    name = "epochs") + 
  labs(colour = "") + 
  geom_hline(yintercept = 0.0067, linetype = "dashed", color = "blue")